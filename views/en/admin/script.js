...
add_seat_btn.addEventListener('click', () => {
	...
	`		    <div class ="mb-3">
			<div class="mb-3">
			    <label for="seat-type-${seat_count}" class="form-label">Seat Type</label>
			    <input type="text" class="form-control" id="seat-type-${seat_count}">
			</div>
			<div class="mb-3">
			    <label for="seat-price-${seat_count}" class="form-label">Price</label>
			    <input type="text" class="form-control" id="seat-price-${seat_count}">
			</div>
			<div class="mb-3">
			    <label for="seat-start-${seat_count}" class="form-label">Seat Start At</label>
			    <input type="text" class="form-control" id="seat-start-${seat_count}">
			</div>
			<div class="mb-3">
			    <label for="seat-end-${seat_count}" class="form-label">Last Seat At</label>
			    <input type="text" class="form-control" id="seat-end-${seat_count}">
			</div>
			<div class="mb-3">
			    <label for="seat-available-${seat_count}" class="form-label">Amount of Seat</label>
...
submit_btn.addEventListener('click', async() => {
	...
	if (submit_result.ok) {
	    ...
	    appendAlert('Concert now added!', 'success');
	} else {
	    let submit_json_result = await submit_result.json();
	    console.error(submit_json_result);
	    appendAlert('Error ' + submit_json_result.message, 'warning');
	}
    } catch(error) {
	appendAlert('Error ' + error.message, 'warning');
...
form_pic.addEventListener('submit', async(e) => {
	...
	if (response.ok) {
	    appendAlert('Image have change', 'success');
	} else {
	    let response_json = await response.json();
	    appendAlert('Error' + response_json.message, 'warning');
	}
    } catch (error) {
	appendAlert('Error' + error.message, 'warning');
	console.error(error.message);
